...
const STATUS_COLOR: Record<Status, { fill: string; stroke: string; text: string; bg: string }> = {
  mastered: { fill: "#34d399", stroke: "#10b981", text: "#34d399", bg: "rgba(52,211,153,0.16)" },
  in_progress: { fill: "#fbbf24", stroke: "#f59e0b", text: "#fbbf24", bg: "rgba(251,191,36,0.16)" },
  target: { fill: "#c084fc", stroke: "#a855f7", text: "#e9d5ff", bg: "rgba(168,85,247,0.2)" },
  required: { fill: "#38bdf8", stroke: "#0ea5e9", text: "#7dd3fc", bg: "rgba(56,189,248,0.16)" },
  outside: { fill: "#64748b", stroke: "#475569", text: "#94a3b8", bg: "rgba(100,116,139,0.16)" },
};
...
export function SkillGraphView({
  ...
  return (
    <div className="space-y-4">
      ...
      {activeNode && (
        <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-border bg-surface/80 p-3.5 backdrop-blur-sm">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-foreground">{activeNode.name}</span>
              <span className="rounded-md border border-border px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-muted">
                {activeNode.category.replace(/-/g, " ")}
              </span>
            </div>
            <p className="mt-0.5 text-xs text-muted">
              Hyperbolic Radius: <span className="text-foreground">{activeNode.radius}</span> · Angle:{" "}
              <span className="text-foreground">{activeNode.angle} rad</span>
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            <div>
              ...
              <span className="font-semibold text-emerald-400">{activeNode.fanOut} skills</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="relative flex flex-col items-center justify-center overflow-hidden rounded-2xl border border-border bg-surface/60 p-4 sm:p-6">
        <div className="relative">
          {/* Ambient Glow */}
          <div className="pointer-events-none absolute inset-8 rounded-full bg-accent/5 blur-3xl" />
        ...
        {/* Disk Info Footer */}
        <div className="mt-4 flex flex-wrap items-center justify-between gap-4 text-xs text-muted w-full max-w-lg border-t border-border/50 pt-3">
...
export function SkillGraphLegend() {
  const items: { status: Status; label: string }[] = [
    ...
    { status: "target", label: "Target skill" },
    { status: "required", label: "Prerequisite path" },
    { status: "outside", label: "Other domain skills" },
  ];
  return (
