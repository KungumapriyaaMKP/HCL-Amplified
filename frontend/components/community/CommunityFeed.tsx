...
export function CommunityFeed({ domain }: { domain: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <div className="text-xs font-bold text-slate-900 mt-1">
                Enrolled Member{memberCount === 1 ? "" : "s"}
              </div>
        ...
        <div className="flex items-center gap-2.5">
          {joined ? (
            <>
              <span className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50/90 px-4 py-2.5 text-xs font-bold text-emerald-700 shadow-2xs">
                <IconCheck className="h-4 w-4 text-emerald-600" />
                <span>Community Member</span>
              </span>
              <button
                onClick={() => setShowCompose(!showCompose)}
                className="inline-flex items-center gap-1.5 rounded-lg bg-[#7C3AED] hover:bg-[#6D28D9] px-5 py-2.5 text-xs font-bold text-white shadow-xs transition-all cursor-pointer"
              >
            ...
            <button
              disabled={joining}
              onClick={join}
              className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-[#7C3AED] via-[#6D28D9] to-[#6366F1] px-6 py-2.5 text-xs sm:text-sm font-bold text-white shadow-xs hover:opacity-95 transition-all cursor-pointer disabled:opacity-50"
            >
        ...
        <div className="rounded-2xl border border-purple-200 bg-white p-5 sm:p-6 shadow-sm space-y-3">
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              onClick={submitPost}
              className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-[#4F46E5] to-[#7C3AED] px-6 py-2.5 text-xs font-bold text-white shadow-xs hover:opacity-95 transition-all cursor-pointer disabled:opacity-50"
            >
      ...
      {!posts ? (
        ...
        <div className="rounded-2xl border border-slate-100 bg-white py-14 px-8 shadow-xs flex flex-col items-center justify-center text-center select-none">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border-2 border-purple-200/90 bg-[#FAF5FF]/40 px-6 py-2.5 text-xs sm:text-sm font-bold text-[#7C3AED] hover:bg-purple-50 shadow-xs transition-all mt-6 cursor-pointer"
          >
        ...
        <div className="space-y-4">
          {posts.map(({ post, authorName, replies }) => (
            <div
              key={post.id}
              className="rounded-2xl border border-slate-100 bg-white p-6 shadow-xs transition-all hover:border-purple-200 hover:shadow-xs"
            >
              <div className="mb-2 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-purple-100 text-xs font-bold text-[#7C3AED] shadow-xs">
                    {authorName[0]?.toUpperCase() || "A"}
                  </div>
                  <div>
                    <span className="text-xs font-bold text-slate-900">{authorName}</span>
                    <span className="text-[10px] text-slate-400 ml-2 font-normal">{formatTime(post.createdAt)}</span>
                  </div>
                </div>
              ...
              <p className="whitespace-pre-wrap text-xs sm:text-sm text-slate-700 leading-relaxed pl-12">{post.content}</p>

              {/* Replies */}
              {replies.length > 0 && (
                <div className="mt-4 space-y-2.5 border-l-2 border-purple-200 pl-4 ml-6">
                  {replies.map(({ reply, authorName: replyAuthor }) => (
                    <div key={reply.id} className="rounded-xl border border-slate-100 bg-slate-50/70 p-3">
                      <div className="mb-1 flex items-center gap-2">
                        ...
                        <span className="text-[9px] text-slate-400 font-normal">{formatTime(reply.createdAt)}</span>
                      </div>
              ...
              {/* Reply Button / Box */}
              {joined && (
                <div className="mt-4 pt-3 border-t border-slate-100 pl-12">
                  {openReplyFor === post.id ? (
                    <div className="space-y-2">
                      <textarea
                        ...
                        className="w-full rounded-lg border border-purple-400 bg-white p-2.5 text-xs text-slate-900 placeholder-slate-400 focus:border-[#7C3AED] focus:outline-none shadow-xs"
                      />
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <button
                          ...
                          className="rounded-lg bg-[#7C3AED] px-4 py-1 text-xs font-bold text-white hover:bg-[#6D28D9] disabled:opacity-50 cursor-pointer shadow-xs"
                        >
