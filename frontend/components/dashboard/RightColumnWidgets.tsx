...
export function YourPlanForToday({ hasGoals = true }: { hasGoals?: boolean }) {
  ...
  return (
    <div className="rounded-sm border border-slate-200/90 bg-white p-5 shadow-2xs space-y-4">
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar pr-1">
        ...
        {tasks.map((task) => (
          ...
          >
            ...
            >
              <div
                className={`w-4.5 h-4.5 rounded-xs border-2 flex items-center justify-center transition-all ${
                  task.completed
                    ? "bg-emerald-500 border-emerald-500 text-white shadow-2xs"
                    : "border-slate-400 bg-white hover:border-purple-600 shadow-2xs"
                }`}
              >
            ...
            <button
              ...
              className="text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-xs p-1 transition-colors cursor-pointer"
              title="Delete task"
            >
      ...
      {/* Add Task Input Form */}
      <form onSubmit={handleAddTask} className="flex items-center gap-2 pt-2 border-t border-slate-200">
        <input
          type="text"
          value={newTaskTitle}
          onChange={(e) => setNewTaskTitle(e.target.value)}
          placeholder="Add a daily study task..."
          className="flex-1 px-3 py-2 rounded-xs border border-slate-300 text-xs font-medium text-slate-900 placeholder-slate-500 focus:outline-hidden focus:border-purple-600 focus:ring-1 focus:ring-purple-600 shadow-2xs transition-all"
          disabled={isSubmitting}
        />
        <button
          type="submit"
          disabled={!newTaskTitle.trim() || isSubmitting}
          className="inline-flex items-center gap-1 px-3.5 py-2 rounded-xs bg-[#6D28D9] hover:bg-[#5B21B6] disabled:opacity-50 text-white text-xs font-extrabold transition-all shadow-xs cursor-pointer"
        >
