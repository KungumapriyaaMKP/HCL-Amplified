          ...
          <div className="relative z-10 mt-6 flex flex-wrap items-end justify-between gap-4">
            ...
            <div>
              <Link
                href={
                  hasGoals
                    ? activeGoal?.nextAction?.moduleId
                      ? `/goals/${activeGoal.id}/modules/${activeGoal.nextAction.moduleId}/practice`
                      : `/goals/${activeGoal?.id}`
                    : "/goals/new"
                }
              ...
              >
                <span>{hasGoals ? "Resume Learning" : "Create a Quest"}</span>
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm sm:text-base font-bold text-slate-900">
              {hasGoals ? "Continue Where You Left Off" : "Recommended Pathways"}
            </h3>
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            {hasGoals && goals.map((goal) => {
              ...
              const domainLower = (goal.domain || "").toLowerCase();
              const isAi = domainLower.includes("ai") || domainLower.includes("machine") || domainLower.includes("llm");
              const isWeb = domainLower.includes("web") || domainLower.includes("react") || domainLower.includes("frontend");
              const isData = domainLower.includes("data") || domainLower.includes("science");
              ...
              const resumeLink = goal.nextAction?.moduleId
                ? `/goals/${goal.id}/modules/${goal.nextAction.moduleId}/practice`
                : `/goals/${goal.id}`;
                ...
                >
                  <div className="flex items-center gap-3.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-purple-700 font-semibold leading-relaxed truncate mt-0.5">
                        {goal.nextAction
                          ? `Last left off: ${goal.nextAction.skillName}`
                  ...
                  <div className="mt-5 flex items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center justify-between text-[11px] font-bold mb-1.5">
                        <span className="text-slate-500 font-bold">
                          {goal.completedModules > 0 ? `${goalPercent}% Complete` : "Ready to continue"}
                        </span>
                    ...
                    <Link
                      href={resumeLink}
                      className="flex h-8 px-3 items-center justify-center gap-1 rounded-xs bg-purple-50 border border-purple-200 text-[#6D28D9] group-hover:bg-[#6D28D9] group-hover:border-[#6D28D9] group-hover:text-white text-xs font-bold transition-all shrink-0 shadow-2xs"
            ...
            {/* If user has 1 goal, show a clean Start New Quest card instead of fake pre-filled pathways */}
            {hasGoals && goals.length === 1 && (
              <Link
                href="/goals/new"
                className="rounded-sm border-2 border-dashed border-slate-200 bg-slate-50/60 p-5 flex flex-col items-center justify-center text-center gap-2 hover:border-purple-300 hover:bg-purple-50/30 transition-all group min-h-[140px]"
              >
                <div className="flex h-10 w-10 items-center justify-center rounded-xs bg-white border border-slate-200 text-slate-500 group-hover:border-purple-300 group-hover:text-[#6D28D9] group-hover:shadow-2xs transition-all">
                  <IconPlus className="h-5 w-5 stroke-[2.2]" />
                </div>
                <div>
                  <div className="text-xs font-bold text-slate-800 group-hover:text-[#6D28D9] transition-colors">
                    Start Another Pathway
                  </div>
                  <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                    Explore and enroll in AI, Cloud, SAP, Web Dev, and 16 more domains
                  </p>
                </div>
              </Link>
            )}

            {/* If user has 0 goals, show starter pathways */}
            {!hasGoals && (
              <>
